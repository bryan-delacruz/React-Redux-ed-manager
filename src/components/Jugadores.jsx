...
const Jugadores = ({ jugadores, agregarTitular, agregarSuplente }) => {
  ...
  useEffect(() => {
    setScrollContainer();
    document.addEventListener("click", setScrollContainer); // eslint-disable-next-line
  }, []);
    ...
    <section>
      ...
      <div className="contenedor-jugadores">
        <div ref={gridJugadores} onClick={() => setScrollContainer.bind(this)}>
          {jugadores.map((e) => (
            <article className="jugador" key={e.id}>
              <img src={e.foto} alt={e.nombre} />
              <h3>{e.nombre}</h3>
              <div>
                <button onClick={() => agregarTitular(e)}>Titular</button>
                <button onClick={() => agregarSuplente(e)}>Suplente</button>
              </div>
            </article>
          ))}
        </div>
